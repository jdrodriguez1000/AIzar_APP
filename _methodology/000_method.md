...
### 46.1. ➕ Por qué Feature y Scenario no usan `F-` ni `S-`

> **Sección añadida al consolidar.** Ver **Anexo A.13**.

`F-` y `S-` son las dos letras más disputadas de cualquier proyecto que además registre su
propio trabajo: `F-` suele estar tomada por *hallazgo* (finding) y `S-` por *sesión*. Un
identificador existe para poder citarse **fuera del archivo donde nació** —en un commit, en un
informe, en una conversación—, y ahí no hay contexto que desambigüe: `F-012` significa una sola
cosa o no significa nada.

Por eso el método reserva **`FT-`** para Feature y **`SC-`** para Scenario, y deja `F-` y `S-`
libres para el registro del proyecto que lo aplique.

⚠️ **Esto cambia la notación, no el método.** La cadena de trazabilidad, su dirección y su regla
son las mismas de §45 y §47.

### 46.2. ➕ La tarea declara su origen

> **Sección añadida al consolidar.** Ver **Anexo A.13**.

**`T-` designa la tarea, y el método no reclama ese prefijo en exclusiva.** Un proyecto que ya
lleve su propia lista de tareas no necesita una segunda: necesita que cada tarea diga de dónde
viene.

Por eso toda tarea declara un **origen**, y es obligatorio:

| Origen | Significado |
|---|---|
| `VS-XXX` | Nace de una Vertical Slice. Es el caso normal del producto |
| *otro* | No nace del producto: trabajo de método, de herramienta, de proceso |

🔑 **Esto no debilita §47, la refuerza.** «Nada se construye sin una razón trazable» pasa de ser
una norma que hay que recordar a un campo que está o no está. Una tarea sin origen es
exactamente la *tarea huérfana* que §47 manda cuestionar, y ahora se detecta mirando, no
juzgando.

⚠️ **Un origen distinto de `VS-XXX` no es una excepción a §47: es su respuesta.** La razón
trazable existe; simplemente no es de producto. Lo que §47 prohíbe es la tarea que no puede
nombrar ninguna.
...
➕ El mecanismo concreto que hace comprobable esta regla es el **origen obligatorio** de cada
tarea (§46.2). *(A.13)*

### 47.1. ➕ Como se registra la cadena: declaracion hacia arriba

> **Sección añadida al consolidar.** Ver **Anexo A.14**.

§45 exige recorrer la cadena en las dos direcciones y §47 prohíbe construir sin razón trazable,
pero ninguna de las tres fuentes dice **dónde se escribe el vínculo**. Una regla sin mecanismo no
es una regla: es una intención, porque nadie puede comprobar si se cumple.

**La regla:**

> **Cada elemento declara únicamente a su padre. Nunca a sus hijos.**

`TC` → `SC` → `FT` → `N`, y `T` → `VS` → `SC`. La necesidad es la raíz y no declara nada.

🔑 **Por qué una sola dirección.** Un hijo conoce a su padre en el momento en que nace; un
padre no conoce a sus hijos futuros. Escribir las dos direcciones crea dos afirmaciones sobre el
mismo vínculo, y el día que uno cambie sin el otro hay que decidir cuál miente.

**Las dos direcciones de §45 se resuelven sin guardar nada:**

| Dirección | Cómo se obtiene |
|---|---|
| Hacia atrás — ¿por qué construimos esto? | Lectura encadenada: cada elemento lleva al siguiente |
| Hacia adelante — ¿cómo se implementó esto? | Búsqueda: quién declara este código como padre |

⚠️ **No se crea un índice de trazabilidad.** Una tabla que repita los vínculos los duplica: la
relación queda escrita en el elemento y en la tabla, y ambas pueden divergir. La cadena **se
recorre, no se guarda**.

🚨 **Con esto §47 pasa a ser comprobable.** Una tarea huérfana ya no es un juicio sobre si el
trabajo «se justifica»: es un elemento cuyo padre está vacío o apunta a algo que no existe. Se
detecta mirando.
...
| **A.13** | ➕ **Feature usa `FT-` y Scenario usa `SC-`**, no `F-` ni `S-`; y **`T-` no es exclusivo del método**: la tarea declara un **origen** obligatorio | ninguna fuente contempla el problema: `005 §27` propone los prefijos sin preguntarse con qué conviven | Adición (§46.1, §46.2). `F-` y `S-` son las letras que todo registro de proyecto ya tiene tomadas —*hallazgo* y *sesión*—, y un identificador que no se puede citar fuera de su archivo no es un identificador. El origen de la tarea convierte §47 en un campo comprobable |
| **A.14** | ➕ **La cadena de trazabilidad se registra por declaracion hacia arriba**: cada elemento nombra solo a su padre, y **no se crea indice de trazabilidad** | ninguna fuente lo contempla: `005 §25–§28` define la cadena y la regla, pero no dónde se escribe el vínculo | Adición (§47.1). Sin mecanismo, §47 no se puede comprobar. Se elige una sola dirección porque dos afirmaciones sobre el mismo vínculo pueden divergir, y un índice central las duplicaría otra vez |
...
**Revisadas con ese criterio las entradas A.8–A.14** (añadidas después de la lista de
...
| **A.13** | Notación y proceso. Los prefijos no restringen ninguna decisión de diseño: el mismo producto, con las mismas Features y los mismos Scenarios, se construye igual se llamen `F-` o `FT-`. El origen de la tarea afecta a cómo se registra el trabajo, no a cómo se construye |
| **A.14** | Registro y proceso. Decide **dónde se escribe** el vínculo, no qué se construye: el mismo producto sale igual con índice central o sin él. Lo que cambia es si la regla de §47 se puede verificar |
